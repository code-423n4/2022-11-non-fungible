    ...
    */
    ...
    const setAddress = async (fnName: string) => {
      it('can be called by owner', async () => {
        await exchange.connect(admin)[fnName](admin.address);
      });
      ...
      it('reverts when address is 0', async () => {
        await expect(
          exchange.connect(admin)[fnName](ZERO_ADDRESS),
    ...
    describe('setBlockRange', async () => {
      it('can be called by owner', async () => {
        await exchange.connect(admin).setBlockRange(5);
      });
      it('reverts when not called by owner', async () => {
        await expect(
    ...
    describe('close', async () => {
      it('can be called by owner', async () => {
        await exchange.connect(admin).close();
      });
      it('reverts when not called by owner', async () => {
    ...
    describe('open', async () => {
      it('can be called by owner', async () => {
        await exchange.connect(admin).open();
      });
      it('reverts when not called by owner', async () => {
